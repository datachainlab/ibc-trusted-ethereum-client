...
import (
	...
	clienttypes "github.com/datachainlab/ibc-trusted-ethereum-client/tests/chains/ethereum/pkg/ibc/client"
...
func (suite *ContractTestSuite) SetupTest() {
	...
	suite.chainA = ethereum.NewChain(suite.T(), 2021, *chainClient, consts.Contract, mnemonicPhrase, uint64(time.Now().UnixNano()))
	suite.chainB = ethereum.NewChain(suite.T(), 2021, *chainClient, consts.Contract, mnemonicPhrase, uint64(time.Now().UnixNano()))
	suite.coordinator = NewCoordinator(suite.T(), suite.chainA, suite.chainB)
}
...
func (suite *ContractTestSuite) TestChannel() {
	...
	chanA, chanB := suite.coordinator.CreateChannel(ctx, chainA, chainB, connA, connB, types.TransferPort, types.TransferPort, channeltypes.UNORDERED)
